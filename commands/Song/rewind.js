...
module.exports = {
	...
	description: "Rewinds the Song by a given amount of Seconds",
	cooldown: 5,
	requiredroles: [],
	alloweduserids: [],
	run: async (client, message, args) => {
		try {
			...
			try {
				...
				message.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`⏪ **Rewound by \`${seekNumber}s\`!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
			} catch (e) {
