	...
	run: async (client, message, args) => {
		try {
			...
			try {
				...
				if (isNaN(songIndex)) return message.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Please provide a valid Song Position!**`).setDescription(`**Usage:**\n> \`${client.settings.get(message.guild.id, "prefix")}remove <Position> [Amount]\``)] });
				if (songIndex < 0 || songIndex >= player.queue.tracks.length) return message.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Position must be between \`1\` and \`${player.queue.tracks.length}\`!**`)] });
				
				const songName = trackTitle(player.queue.tracks[songIndex]);
				await player.queue.splice(songIndex, amount);
				
				message.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`🗑 **Removed${amount > 1 ? ` ${amount} Songs starting from` : ``} \`${songName}\` from the Queue!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
			} catch (e) {
