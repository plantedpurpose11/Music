		...
		try {
			...
			try {
				...
				let embed = new MessageEmbed().setColor(ee.color)
					...
					.addFields({ name: `Autoplay:`, value: `>>> ${autoplay ? `${client.allEmojis.check_mark}` : `${client.allEmojis.x}`}`, inline: true })
					.addFields({ name: `DJ-Role(s):`, value: `>>> ${djs}`, inline: true })
