	...
	run: async (client, message, args) => {
		try {
			...
			try {
				...
				// Store pre-shuffle order for unshuffle
				client.maps.set(`beforeshuffle-${guildId}`, player.queue.tracks.map(t => t));
				await player.queue.shuffle();
				
				message.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`🔀 **Shuffled ${player.queue.tracks.length} Songs!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
			} catch (e) {
