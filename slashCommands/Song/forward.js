...
module.exports = {
	name: "forward", description: "Forwards the Song", cooldown: 5, requiredroles: [], alloweduserids: [],
	options: [{ "Integer": { name: "seconds", description: "Seconds to forward", required: false } }],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				const seekNumber = interaction.options.getInteger("seconds") || 10;
				let seektime = player.position + (seekNumber * 1000);
				const duration = trackDuration(cur);
				if (seektime >= duration) seektime = duration - 1000;
				await player.seek(seektime);
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`⏩ **Forwarded by \`${seekNumber}s\`!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
