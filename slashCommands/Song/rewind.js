...
module.exports = {
	name: "rewind", description: "Rewinds the Song", cooldown: 5, requiredroles: [], alloweduserids: [],
	options: [{ "Integer": { name: "seconds", description: "Seconds to rewind", required: false } }],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`⏪ **Rewound by \`${seekNumber}s\`!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
