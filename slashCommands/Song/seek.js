...
module.exports = {
	name: "seek", description: "Seeks to a position in the Song", cooldown: 5, requiredroles: [], alloweduserids: [],
	options: [{ "Integer": { name: "seconds", description: "Seconds to seek to", required: true } }],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				const seekNumber = interaction.options.getInteger("seconds");
				const duration = trackDuration(cur);
				if (seekNumber * 1000 > duration) return interaction.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Cannot seek beyond the Song duration!**`)], ephemeral: true });
				await player.seek(seekNumber * 1000);
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`⏩ **Seeked to \`${seekNumber}s\`!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
