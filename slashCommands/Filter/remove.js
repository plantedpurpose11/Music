...
module.exports = {
	name: "removefilter", description: "Removes a Filter", cooldown: 5, requiredroles: [], alloweduserids: [],
	options: [{ "StringChoices": { name: "filter", description: "Filter to remove", required: true, choices: AVAILABLE_FILTERS.map(f => [f, f]) } }],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				if (!activeFilters.includes(filterName)) return interaction.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Filter \`${filterName}\` is not active!**`)], ephemeral: true });
				...
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`🎛 **Removed filter \`${filterName}\`!**`).setDescription(`Active: ${activeFilters.length > 0 ? activeFilters.map(f => `\`${f}\``).join(", ") : "None"}`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
