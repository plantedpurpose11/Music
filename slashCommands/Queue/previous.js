...
module.exports = {
	name: "previous", description: "Plays the previous Song", cooldown: 5, requiredroles: [], alloweduserids: [], options: [],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				if (!prev || (Array.isArray(prev) && prev.length === 0)) return interaction.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **No previous Song!**`)], ephemeral: true });
				const prevTrack = Array.isArray(prev) ? prev[prev.length - 1] : prev;
				await player.queue.add(prevTrack, 0);
				await player.skip();
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`⏮ **Playing previous Song!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
