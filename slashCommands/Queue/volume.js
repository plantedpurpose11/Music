...
module.exports = {
	name: "volume", description: "Sets the Volume", cooldown: 5, requiredroles: [], alloweduserids: [],
	options: [{ "Integer": { name: "volume", description: "Volume (0-150)", required: true } }],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				if (volume < 0 || volume > 150) return interaction.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Volume must be between 0 and 150!**`)], ephemeral: true });
				await player.setVolume(volume);
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`🔊 **Volume set to \`${volume}%\`!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
