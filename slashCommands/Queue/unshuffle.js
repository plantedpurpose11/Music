...
module.exports = {
	name: "unshuffle", description: "Restores the Queue to pre-shuffle order", cooldown: 5, requiredroles: [], alloweduserids: [], options: [],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				const saved = client.maps.get(`beforeshuffle-${guildId}`);
				if (!saved) return interaction.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **No shuffle history found!**`)], ephemeral: true });
				await player.queue.splice(0, player.queue.tracks.length);
				for (const track of saved) await player.queue.add(track);
				client.maps.delete(`beforeshuffle-${guildId}`);
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`🔀 **Unshuffled the Queue!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
