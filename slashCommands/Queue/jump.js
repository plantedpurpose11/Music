...
module.exports = {
	name: "jump", description: "Jumps to a specific Song in the Queue", cooldown: 5, requiredroles: [], alloweduserids: [],
	options: [{ "Integer": { name: "position", description: "Position in the Queue to jump to", required: true } }],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				if (Position < 1 || Position > player.queue.tracks.length) return interaction.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Position must be between 1 and ${player.queue.tracks.length}!**`)], ephemeral: true });
				await player.skip(Position);
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`⏭ **Jumped to Position \`${Position}\`!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
