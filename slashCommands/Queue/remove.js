...
module.exports = {
	name: "remove", description: "Removes a Song from the Queue", cooldown: 5, requiredroles: [], alloweduserids: [],
	options: [{ "Integer": { name: "position", description: "Song position to remove", required: true } }, { "Integer": { name: "amount", description: "Amount of Songs to remove", required: false } }],
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				if (pos < 0 || pos >= player.queue.tracks.length) return interaction.reply({ embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Invalid position!**`)], ephemeral: true });
				await player.queue.splice(pos, amount);
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`🗑 **Removed \`${amount}\` Song(s) from position \`${pos + 1}\`!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
