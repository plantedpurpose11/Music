	...
	run: async (client, interaction) => {
		try {
			...
			try {
				...
				interaction.reply({ embeds: [new MessageEmbed().setColor(ee.color).setTimestamp().setTitle(`${autoplay ? "✅" : "❌"} **Autoplay is now \`${autoplay ? "ON" : "OFF"}\`!**`).setFooter({ text: `Action by: ${member.user.tag}`, iconURL: member.user.displayAvatarURL({dynamic: true}) })] })
			} catch (e) {
